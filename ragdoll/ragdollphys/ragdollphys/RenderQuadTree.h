...
public:
	...
	void render(Matrix4* camera, Vector3f* camera_pos, Quaternion* camera_orientation, float cameraAngle)
	{
		...
		auto range = scene->query_render_range(scene_bounds, camera, camera_pos, camera_orientation, cameraAngle);
		for (int i = 0; i < range->size(); i++)
		{
			auto scene_element = range->at(i);

			scene_element->render(camera, camera_pos, camera_orientation, cameraAngle);
		}
		scene_bounds->render(camera, camera_pos, camera_orientation, cameraAngle);
	}
};
