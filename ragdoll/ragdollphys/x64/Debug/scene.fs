...
void main()
{
	...
	vec4 result = texture(ourTexture, vec2(TexCoord.x,TexCoord.y));
	...
	vec3 objectColor = vec3(0.3, 0.0, 0.0);
	float diff = max(dot(normal, lightDir), 0.0);
	vec3 diffuse = diff * lightColor + result.rgb;
	vec3 shader_result = (ambient + diffuse);
	
	FragColor = vec4(vec3(shader_result.r, shader_result.g, shader_result.b), result.a);
	//FragColor = vec4(texture(ourTexture, TexCoord).rgb  * ourColor.rgb, 1.0);
	//FragColor = ourColor;
} 